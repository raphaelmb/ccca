  ...
  implements StockEntryRepository
{
  ...
  async save(stockEntry: StockEntry): Promise<void> {
    await this.connection.query(
      "insert into ccat9.stock_entry (id_product, operation, quantity) values ($1, $2, $3)",
  ...
  async getByIdProduct(idProduct: number): Promise<StockEntry[]> {
    const stockEntriesData = await this.connection.query(
      "select * from ccat9.stock_entry where id_product = $1",
      [idProduct]
    );
  ...
  async clean(): Promise<void> {
    await this.connection.query("delete from ccat9.stock_entry", []);
  }
}
